...
# =============================================================================
# DATA PROVENANCE TABLE (for Tab 3)
# =============================================================================
DATA_PROVENANCE_ROWS = [
    ...
    (18, "Lighting", "Number of Lights", "Installed fixtures", "User Input",
     "User input", "User-defined", "No default"),
    (19, "Lighting", "Wattage per Light", "Lamp power", "User Input",
     "User input (W)", "User-defined", "LED–incandescent"),
    (20, "Lighting", "Daily Usage Hours", "Average daily use", "User Input",
     "User input (h/day)", "User-defined", "Early-stage"),
    ...
    (22, "Water", "Toilet Flush Volume", "Water per flush", "Assumption / User",
     "Single=9; Dual std=5; Dual eff=4; Custom", "BRANZ (2023)", "NZ fixtures"),
    ...
    (25, "Water", "Laundry Water per Cycle", "Washing machine demand", "Assumption / User",
     "User input (L/cycle)", "BRANZ (2023)", ""),
    (26, "Water", "Dishwasher Water per Cycle", "Dishwasher demand", "Assumption / User",
     "User input (L/cycle)", "BRANZ (2023)", ""),
    ...
    (28, "Carbon", "Grid Emission Factor", "Carbon intensity of grid", "Constant",
     "0.0729 kgCO₂e/kWh (default)", "MfE (2024)", "Location-based"),
    (29, "Carbon", "Water Emissions", "CO₂ from water supply", "Calculated",
     "Water × factor", "MfE (2024)", ""),
    ...
    (33, "Cost (Opex)", "Annual Operating Cost", "Total operating cost", "Calculated",
     "Energy + water", "Derived", ""),
    (34, "Cost (Capex)", "Floor Insulation Cost", "Installed floor insulation", "Assumption / User",
     "0/20/40/70/110 NZD/m²", "Market benchmark", "Early-stage"),
    (35, "Cost (Capex)", "Roof Insulation Cost", "Installed roof insulation", "Assumption / User",
     "0/15/25/35/35 NZD/m²", "Market benchmark", ""),
    (36, "Cost (Capex)", "Wall Insulation Cost", "Installed wall insulation", "Assumption / User",
     "0/25/45/75/120 NZD/m²", "Market benchmark", ""),
    (37, "Cost (Capex)", "Window Cost", "Installed glazing", "Assumption / User",
     "300/600/950/1400 NZD/m²", "Market benchmark", ""),
    (38, "Cost (Capex)", "Space Heating System Cost", "Installed heating system", "Assumption / User",
     "0/1500/4500/7000 NZD", "Market benchmark", ""),
    (39, "Cost (Capex)", "Water Heating System Cost", "Installed DHW system", "Assumption / User",
     "0/3500/6500 NZD", "Market benchmark", ""),
    (40, "Cost (Capex)", "Water Fixture Costs", "Toilet/shower/tap upgrades", "Assumption / User",
     "As specified", "Market benchmark", ""),
    (41, "Metrics", "Annual Savings", "Opex reduction", "Calculated",
     "Baseline − option", "Derived", ""),
...
def jump_to_results():
    # Scroll the main page to top so right-side results are immediately visible.
    components.html(
        """
        <script>
...
def calculate_water_heating_from_enduse(s: dict, enduse_L_y: dict) -> dict:
    # User-editable hot water fractions (baseline/option specific)
...
def init_defaults():
    # Flow flags
    st.session_state.setdefault("baseline_ready", False)   # baseline mode activated
    st.session_state.setdefault("option_unlocked", False)  # show option panel
    st.session_state.setdefault("option_seeded", False)    # option copied once
    st.session_state.setdefault("compare_ready", False)    # compare mode activated

    # Cached outputs (optional)
...
def reset_scenario(prefix: str):
    # Reset only the scenario inputs back to defaults (flow flags remain)
    # coefficients
...
def apply_code_minimum(prefix: str):
    # Keep location "any location" (do not touch closestCity)
    # Envelope
    ...
    # Fixtures: fill with baseline-standard selections (fix your prior complaint)
...
HELP = {
    ...
    "lighting": "Lighting electricity = count × watts × hours/day × 365. Early-stage placeholder.",
    "tariffs": "Tariffs/factors are scenario-specific (region/provider dependent). Adjust to your local bill.",
    "efs": "Defaults from MfE (2024) guidance (grid-average; water supply/wastewater factor).",
    "hw_frac": "Fraction of end-use water assumed to be hot water (0–1). Toilets excluded.",
}

# =============================================================================
# UI: captions directly below selections
# =============================================================================
...
def show_envelope_caption(element: str, label: str):
    if label in (PLACEHOLDER, None):
        return
    if label == "Custom":
        # custom captions are shown with the custom inputs
        return
...
# =============================================================================
# INPUT PANELS (3 expanders; each with 2 internal columns)
# =============================================================================
def scenario_panel(prefix: str, title: str, show_compare_controls: bool):
    ...
    # Expander 1: Core + Climate + Lighting (2 columns)
    ...
    # Expander 2: Envelope + Systems (2 columns)
    with st.expander("Envelope + Systems", expanded=False):
    ...
    # Expander 3: Water + Assumptions (2 columns)
    with st.expander("Water + Usage + Tariffs + Emissions", expanded=False):
        wc1, wc2 = st.columns(2, gap="small")

        with wc1:
            st.markdown("**Fixtures + appliance water**")

            select_with_placeholder("Toilet type", TOILET_OPTS, key=f"{prefix}_toiletType")
            ...
            select_with_placeholder("Shower type", SHOWER_OPTS, key=f"{prefix}_showerType")
            ...
            select_with_placeholder("Tap type", TAP_OPTS, key=f"{prefix}_tapType")
...
# If baseline activated and option unlocked but not seeded, seed BEFORE rendering option widgets
...
with tab_calc:
    ...
    # ---------- Inputs ----------
    with left:
        scenario_panel("b", "Baseline", show_compare_controls=False)

        # Gate: Calculate baseline once to activate baseline mode & unlock option
        ...
        # Option panel appears only after baseline is activated
        if st.session_state.get("option_unlocked", False):
            st.divider()
            scenario_panel("o", "Option", show_compare_controls=True)

            # Gate: Calculate & compare once to activate compare mode
    ...
    with right:
        ...
        # Use a scrollable container so results can scroll independently
        try:
        ...
        with results_box:
            if not st.session_state.get("baseline_ready", False):
                ...
                # Always recompute baseline live once activated (if valid)
                b_now = get_scenario("b")
                missing_b = validate_scenario(b_now)
                if missing_b:
                    ...
                    if not st.session_state.get("compare_ready", False):
                        ...
                        # Always recompute option live once compare activated (if valid)
                        o_now = get_scenario("o")
                        missing_o = validate_scenario(o_now)
                        if missing_o:
