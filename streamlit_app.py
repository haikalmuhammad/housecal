...
import streamlit.components.v1 as components# streamlit_app.py
import copy
import json
import math
from datetime import datetime
...
DATA_PROVENANCE_ROWS = [
    ...
    (18, "Water", "Total Water Use", "Annual indoor water use", "Calculated",
     "Sum of end uses", "Derived", "m³/year"),
    (19, "Carbon", "Grid Emission Factor", "Carbon per kWh electricity", "Constant",
     "0.0729 kgCO₂e/kWh (default)", "MfE (2024)", "2023 value"),
    (20, "Carbon", "Water Emission Factor", "Carbon per m³ water", "Constant",
     "0.0349 kgCO₂e/m³ (default)", "MfE (2024)", ""),
    (21, "Cost (Opex)", "Electricity Tariff", "Retail electricity price", "Default / User",
     "Default=0.312 NZD/kWh (editable)", "Electricity Authority (2024)", ""),
    (22, "Cost (Opex)", "Water Tariff", "Residential water price", "Default / User",
     "Default=2.296 NZD/m³ (editable)", "Auckland Council (2025)", ""),
]
...
def calculate_scenario(s: dict, coeffs: dict) -> dict:
    ...
    return {
        ...
        "waterConsumption_m3_y": water_use["V_total_m3_y"],
        "carbon_kgco2e_y": carbon["CO2_total_kg_y"],
        "opex_nzd_y": opex["opex_total_nzd_y"],
        "capex_nzd": capex["capex_total_nzd"],
    }
...
def init_defaults():
    ...
    # categorical defaults (start unselected)
    cat_keys = [
...
def reset_scenario(prefix: str):
    # keep it simple: reset readiness if baseline reset
    if prefix == "b":
        st.session_state["baseline_ready"] = False
        st.session_state["compare_ready"] = False
        st.session_state["option_unlocked"] = False
        st.session_state["option_seeded"] = False
        st.session_state["baseline_results"] = None
        st.session_state["option_results"] = None

    st.session_state[f"{prefix}_coef_grid_ef"] = float(LOOKUP["constants"]["grid_emission_factor_kgco2e_per_kwh"])
    st.session_state[f"{prefix}_coef_water_ef"] = float(LOOKUP["constants"]["water_emission_factor_kgco2e_per_m3"])
    st.session_state[f"{prefix}_coef_elec_tariff"] = float(LOOKUP["constants"]["electricity_tariff_nzd_per_kwh_default"])
    st.session_state[f"{prefix}_coef_water_tariff"] = float(LOOKUP["constants"]["water_tariff_nzd_per_m3_default"])

    st.session_state[f"{prefix}_floorArea"] = float(LOOKUP["defaults"]["core"]["floorArea"])
    st.session_state[f"{prefix}_ceilingHeight"] = float(LOOKUP["defaults"]["core"]["ceilingHeight"])
    st.session_state[f"{prefix}_householdSize"] = int(LOOKUP["defaults"]["core"]["householdSize"])
    st.session_state[f"{prefix}_windowArea"] = float(LOOKUP["defaults"]["core"]["windowArea"])

    st.session_state[f"{prefix}_light_n"] = int(LOOKUP["defaults"]["lighting"]["numberOfLights"])
    st.session_state[f"{prefix}_light_watts"] = float(LOOKUP["defaults"]["lighting"]["wattsPerLight"])
    st.session_state[f"{prefix}_light_hours"] = float(LOOKUP["defaults"]["lighting"]["hoursPerDay"])

    st.session_state[f"{prefix}_wash_has"] = "No"
    st.session_state[f"{prefix}_wash_cycles"] = float(LOOKUP["defaults"]["washing_machine"]["cyclesPerWeek"])
    st.session_state[f"{prefix}_wash_L"] = float(LOOKUP["defaults"]["washing_machine"]["waterPerCycle_L"])
    st.session_state[f"{prefix}_dish_has"] = "No"
    st.session_state[f"{prefix}_dish_cycles"] = float(LOOKUP["defaults"]["dishwasher"]["cyclesPerWeek"])
    st.session_state[f"{prefix}_dish_L"] = float(LOOKUP["defaults"]["dishwasher"]["waterPerCycle_L"])

    st.session_state[f"{prefix}_closestCity"] = PLACEHOLDER
    st.session_state[f"{prefix}_use_custom_hdd"] = False
    st.session_state[f"{prefix}_hdd_override_value"] = 2000.0
    ...
    for k in [
        ...
        st.session_state[f"{prefix}_{k}"] = PLACEHOLDER
...
# INPUT PANELS
# =============================================================================
def scenario_panel(prefix: str, title: str, CITIES, ROOF_OPTS, WALL_OPTS, FLOOR_OPTS, WIN_OPTS, SPACE_SYS_OPTS, WATER_SYS_OPTS, TOILET_OPTS, SHOWER_OPTS, TAP_OPTS):
    st.markdown(f"### {title}")
    ...
    with st.expander("Core climate + lighting", expanded=True):
        ...
        with cc1:
            ...
            if st.session_state[f"{prefix}_use_custom_hdd"]:
                st.number_input(
                    "Custom HDD (base 18°C)", min_value=0.0, max_value=6000.0, step=50.0,
                    key=f"{prefix}_hdd_override_value", help=HELP["hdd_value"]
                )
...
def _card_html(title: str, unit: str, base: str, opt: str, delta: str, payback_only: bool = False):
    if payback_only:
        rows = f"""
        <div class="kpi-grid">
          <div class="kpi-row"><div class="kpi-label">Simple payback</div><div class="kpi-val">{delta}</div></div>
          <div class="kpi-row"><div class="kpi-label">Formula</div><div class="kpi-val">(ΔCapex) ÷ (−ΔOpex)</div></div>
        </div>
        """
    else:
        rows = f"""
        <div class="kpi-grid">
          <div class="kpi-row"><div class="kpi-label">Baseline</div><div class="kpi-val">{base}</div></div>
          <div class="kpi-row"><div class="kpi-label">Option</div><div class="kpi-val">{opt}</div></div>
          <div class="kpi-row"><div class="kpi-label">Δ (Option − Base)</div><div class="kpi-val">{delta}</div></div>
        </div>
        """
...
def render_kpi_cards(b_res: dict | None, o_res: dict | None):
    # metrics: 3 rows x 2 cols
    metrics = [
        ("Total energy use", "kWh/year", "totalElectricity_kwh_y", 1, "num"),
        ("Total water use", "m³/year", "waterConsumption_m3_y", 2, "num"),
        ("Operational carbon", "kgCO₂e/year", "carbon_kgco2e_y", 1, "num"),
        ("Operational expenditure", "NZD/year", "opex_nzd_y", 0, "money"),
        ("Capital expenditure", "NZD (one-off)", "capex_nzd", 0, "money"),
        ("Simple payback years", "years", "payback", 1, "payback"),
    ]

    def fmt_val(v, dec, mode):
        if v is None:
            return "—"
        if mode == "money":
            return fmt_money(float(v), dec)
        return fmt_num(float(v), dec)

    # compute payback based on b/o capex + opex
    pb = None
    if b_res is not None and o_res is not None:
        inc_capex = float(o_res["capex_nzd"]) - float(b_res["capex_nzd"])
        savings = float(b_res["opex_nzd_y"]) - float(o_res["opex_nzd_y"])
        if inc_capex <= 0:
            pb = 0.0
        elif savings <= 0:
            pb = None
    ...
    # render 6 cards in 3 rows x 2 cols
    for r in range(3):
        cols = st.columns(2, gap="small")
        for c in range(2):
            i = r * 2 + c
            title, unit, key, dec, mode = metrics[i]

            if mode == "payback":
                pb_txt = fmt_num(pb, dec) if pb is not None else "—"
                html = _card_html(title, unit, "—", "—", pb_txt, payback_only=True)
                cols[c].markdown(html, unsafe_allow_html=True)
                continue

            b_v = float(b_res[key]) if b_res is not None else None
            o_v = float(o_res[key]) if o_res is not None else None
            d_v = (o_v - b_v) if (o_v is not None and b_v is not None) else None

            base_txt = fmt_val(b_v, dec, "money" if mode == "money" else "num")
            opt_txt = fmt_val(o_v, dec, "money" if mode == "money" else "num")

            if mode == "money":
                delta_txt = fmt_money(d_v, dec) if d_v is not None else "—"
            else:
                delta_txt = fmt_num(d_v, dec) if d_v is not None else "—"

            html = _card_html(title, unit, base_txt, opt_txt, delta_txt, payback_only=False)
            cols[c].markdown(html, unsafe_allow_html=True)
...
def render_data_sources_tab():
    st.header("Data sources (full table)")
    df = pd.DataFrame(
        DATA_PROVENANCE_ROWS,
        columns=[
...
st.caption("Baseline → Calculate → unlock Option → Calculate & Compare. Inputs and Results are scroll-contained for a 16:9 1080p workflow.")
...
with tab_calc:
    ...
    with left:
        ...
        try:
            ...
            baseline_box = st.container()

        with baseline_box:
            scenario_panel(
                "b", "Baseline",
                CITIES, ROOF_OPTS, WALL_OPTS, FLOOR_OPTS, WIN_OPTS,
                SPACE_SYS_OPTS, WATER_SYS_OPTS, TOILET_OPTS, SHOWER_OPTS, TAP_OPTS
            )
            ...
            if not st.session_state["baseline_ready"]:
                ...
                if st.button("Calculate Baseline", use_container_width=True, disabled=bool(missing_b)):
                    try:
                        ...
                        st.error("Baseline calculation failed.")
                        st.exception(e)
        ...
        if st.session_state.get("option_unlocked", False):
            ...
            try:
                option_box = st.container(height=INPUT_BOX_H, border=True)
            except TypeError:
                option_box = st.container()
                ...
                if not st.session_state.get("compare_ready", False):
                    ...
                    if st.button("Calculate & Compare", use_container_width=True, disabled=bool(missing_o)):
                        try:
                            ...
                            st.error("Option calculation failed.")
                            st.exception(e)

    # ---- RIGHT: results in one scroll container; title INSIDE box; cards layout 3x2
    with right:
        # shorter so it fits 1080p (and still allows scrolling for details)
        RESULTS_BOX_H = 680

        try:
            results_box = st.container(height=RESULTS_BOX_H, border=True)
        except TypeError:
        ...
        with results_box:
            ...
            b_res = st.session_state.get("baseline_results", None)
            o_res = st.session_state.get("option_results", None)

            # Live update baseline after first calc (optional but matches your “changes take effect” behavior)
            try:
                if st.session_state.get("baseline_ready", False):
                    b_now_live = get_scenario("b")
                    if not validate_scenario(b_now_live):
                        b_res = calculate_scenario(b_now_live, get_coeffs("b"))
                        st.session_state["baseline_results"] = b_res

                # Live update option after compare ready
                if st.session_state.get("compare_ready", False):
                    o_now_live = get_scenario("o")
                    if not validate_scenario(o_now_live):
                        o_res = calculate_scenario(o_now_live, get_coeffs("o"))
                        st.session_state["option_results"] = o_res

            except Exception as e:
                st.error("Results rendering failed.")
                st.exception(e)

            if not st.session_state.get("baseline_ready", False):
                st.info("Fill Baseline inputs and click **Calculate Baseline** to show the Baseline cards.")
            else:
                # After baseline calc: show same card set, but Option/Delta will be blank until compare_ready
                if not st.session_state.get("compare_ready", False):
                    st.caption("Baseline calculated. Option is unlocked. When you click **Calculate & Compare**, the same cards will populate Option + Δ.")
                render_kpi_cards(b_res, o_res)

            st.divider()

            # Export still available (optional)
            if st.session_state.get("baseline_ready", False) and b_res is not None:
                payload = {
                    "timestamp": datetime.utcnow().isoformat() + "Z",
                    "baseline": {"coefficients": get_coeffs("b"), "results": b_res},
                    "option": {"coefficients": get_coeffs("o"), "results": o_res} if o_res is not None else None,
                    "notes": {
...
# Fit typical 16:9 1080p layouts (tweak if you want tighter/looser)
PANEL_HEIGHT_PX = 760  # left input scroll box + right results box
RESULTS_HEIGHT_PX = 760
...
def reset_scenario(prefix: str):
    ...
    st.session_state[f"{prefix}_roofR_custom"] = 6.6
    st.session_state[f"{prefix}_roofCost_custom"] = 25.0
    st.session_state[f"{prefix}_wallR_custom"] = 2.0
    st.session_state[f"{prefix}_wallCost_custom"] = 45.0
    st.session_state[f"{prefix}_floorR_custom"] = 2.0
    st.session_state[f"{prefix}_floorCost_custom"] = 40.0
    st.session_state[f"{prefix}_windowU_custom"] = 3.0
    st.session_state[f"{prefix}_windowCost_custom"] = 600.0

    st.session_state[f"{prefix}_spaceCOP_custom"] = 2.5
    st.session_state[f"{prefix}_spaceInstall_custom"] = 4500.0
    st.session_state[f"{prefix}_waterCOP_custom"] = 2.0
    st.session_state[f"{prefix}_waterInstall_custom"] = 6500.0

    st.session_state[f"{prefix}_toilet_value_custom"] = 5.0
    st.session_state[f"{prefix}_toilet_cost_custom"] = 450.0
    st.session_state[f"{prefix}_shower_value_custom"] = 7.0
    st.session_state[f"{prefix}_shower_cost_custom"] = 120.0
    st.session_state[f"{prefix}_tap_value_custom"] = 6.0
    st.session_state[f"{prefix}_tap_cost_custom"] = 150.0

    du = LOOKUP["defaults"]["usage"]
    st.session_state[f"{prefix}_hotWater_setpoint_C"] = float(du["hotWater_setpoint_C"])
    st.session_state[f"{prefix}_coldWater_inlet_C"] = float(du["coldWater_inlet_C"])
    st.session_state[f"{prefix}_toiletFlushes_ppd"] = float(du["toiletFlushes_per_person_day"])
    st.session_state[f"{prefix}_showers_ppd"] = float(du["showers_per_person_day"])
    st.session_state[f"{prefix}_minutes_per_shower"] = float(du["minutes_per_shower"])
    st.session_state[f"{prefix}_tapMinutes_ppd"] = float(du["tapMinutes_per_person_day"])

    hw = du["hot_water_fractions"]
    st.session_state[f"{prefix}_hw_frac_shower"] = float(hw["shower"])
    st.session_state[f"{prefix}_hw_frac_tap"] = float(hw["tap"])
    st.session_state[f"{prefix}_hw_frac_laundry"] = float(hw["laundry"])
    st.session_state[f"{prefix}_hw_frac_dishwasher"] = float(hw["dishwasher"])
...
# =============================================================================
# RESULTS: KPI CARDS (3 rows x 2 columns)
# =============================================================================
def _safe_delta(b, o):
...
def _kpi_value(res: dict, key: str):
    # Small indirection in case you later refactor fields
    return res.get(key)

def _compute_simple_payback_years(b_res: dict, o_res: dict):
    ...
    if inc_capex <= 0:
        return 0.0, "No additional capex (option ≤ baseline capex)."
    if savings <= 0:
        return None, "No payback (opex savings ≤ 0)."
    return inc_capex / savings, ""
...
def render_kpi_cards(b_res: dict | None, o_res: dict | None):
    ...
    def card_html(title, unit, b, o, d, note=""):
        def _v(x): return x if x is not None else "—"
        delta_arrow = ""
        if d is not None:
            delta_arrow = " " + direction_arrow(d)
        return f"""
          <div class="kpi-card">
            <div class="kpi-title">{title}</div>
            <div class="kpi-unit">{unit}</div>
            <div class="kpi-row"><div class="kpi-label">Baseline</div><div class="kpi-val">{_v(b)}</div></div>
            <div class="kpi-row"><div class="kpi-label">Option</div><div class="kpi-val">{_v(o)}</div></div>
            <div class="kpi-row"><div class="kpi-label">Δ (Option − Baseline)</div><div class="kpi-val kpi-delta">{_v(d)}{delta_arrow}</div></div>
            {f'<div class="kpi-foot">{note}</div>' if note else ''}
          </div>
        """

    if b_res is None:
        st.info("Fill Baseline inputs and click **Calculate Baseline** to show results.")
        return

    # Baseline values
    b_energy = b_res["totalElectricity_kwh_y"]
    b_water = b_res["waterConsumption"]["V_total_m3_y"]
    b_carbon = b_res["carbon"]["CO2_total_kg_y"]
    b_opex = b_res["opex"]["opex_total_nzd_y"]
    b_capex = b_res["capex"]["capex_total_nzd"]

    # Option values (optional)
    o_energy = o_water = o_carbon = o_opex = o_capex = None
    if o_res is not None:
        o_energy = o_res["totalElectricity_kwh_y"]
        o_water = o_res["waterConsumption"]["V_total_m3_y"]
        o_carbon = o_res["carbon"]["CO2_total_kg_y"]
        o_opex = o_res["opex"]["opex_total_nzd_y"]
        o_capex = o_res["capex"]["capex_total_nzd"]

    # Deltas (optional)
    d_energy = _safe_delta(b_energy, o_energy)
    d_water = _safe_delta(b_water, o_water)
    d_carbon = _safe_delta(b_carbon, o_carbon)
    d_opex = _safe_delta(b_opex, o_opex)
    d_capex = _safe_delta(b_capex, o_capex)

    # Payback
    pb_val = None
    pb_note = ""
    if o_res is not None:
        pb_val, pb_note = _compute_simple_payback_years(b_res, o_res)

    pb_display = fmt_num(pb_val, 1) if pb_val is not None else "—"
    inc_capex = (o_capex - b_capex) if (o_capex is not None) else None
    dec_opex = (b_opex - o_opex) if (o_opex is not None) else None
    pb_formula = ""
    if o_res is not None:
        pb_formula = f"Payback = (Δ Capex) / (↓ Opex) = {fmt_money(inc_capex) if inc_capex is not None else '—'} / {fmt_money(dec_opex) if dec_opex is not None else '—'}"
    ...
    html += card_html(
        ...
        fmt_num(b_energy, 1),
        fmt_num(o_energy, 1) if o_energy is not None else None,
        fmt_num(d_energy, 1) if d_energy is not None else None,
    )
    html += card_html(
        "Total water use", "m³/year",
        fmt_num(b_water, 2),
        fmt_num(o_water, 2) if o_water is not None else None,
        fmt_num(d_water, 2) if d_water is not None else None,
    )
    html += card_html(
        "Operational carbon", "kgCO₂e/year",
        fmt_num(b_carbon, 1),
        fmt_num(o_carbon, 1) if o_carbon is not None else None,
        fmt_num(d_carbon, 1) if d_carbon is not None else None,
    )
    html += card_html(
        "Operational expenditure", "NZD/year",
        fmt_money(b_opex, 0),
        fmt_money(o_opex, 0) if o_opex is not None else None,
        fmt_money(d_opex, 0) if d_opex is not None else None,
    )
    html += card_html(
        "Capital expenditure", "NZD (total)",
        fmt_money(b_capex, 0),
        fmt_money(o_capex, 0) if o_capex is not None else None,
        fmt_money(d_capex, 0) if d_capex is not None else None,
    )
    html += card_html(
        "Simple payback years", "years",
        "—" if o_res is None else "n/a",
        "—" if o_res is None else "n/a",
        pb_display if o_res is not None else "—",
        note=(pb_formula + (f" · {pb_note}" if pb_note else "")) if o_res is not None else "Payback appears after Option is calculated.",
    )
    html += "</div>"
...
st.title("NZ Housing Sustainability Calculator (Prototype)")
st.caption(
    "Early-stage comparison tool. Simplified, indicative, non-certification. "
    "Baseline → unlock Option → Compare. After activation, results update automatically as you edit inputs."
)
...
with tab_calc:
    ...
    # LEFT: put inputs inside a scroll box
    with left:
        try:
            input_box = st.container(height=PANEL_HEIGHT_PX, border=True)
        except TypeError:
        ...
        with input_box:
            ...
            if st.session_state.get("option_unlocked", False):
                ...
                if not st.session_state.get("compare_ready", False):
                    ...
                    st.success("Comparison activated. Results update live as you edit.")

    # RIGHT: results inside a scroll box, with the title inside that box
    with right:
        try:
            results_box = st.container(height=RESULTS_HEIGHT_PX, border=True)
        except TypeError:
            results_box = st.container()

        with results_box:
            st.markdown("## Results")  # 1) title inside the box
            ...
            # 3) baseline cards appear immediately after Calculate Baseline,
            #    and then those same cards show option+delta after Calculate & Compare.
            render_kpi_cards(b_res, o_res)

            # Optional: keep your deep-dive visuals, but hide them by default so the box stays "short"
            with st.expander("Details (charts + export)", expanded=False):
                if b_res is None:
                    st.info("Calculate Baseline first.")
                elif o_res is None:
                    st.info("Calculate & Compare to see comparative charts and export.")
                else:
                    st.markdown("### Charts (all vertical bars)")

                    df_kpi = pd.DataFrame([
                        {"Metric": "Energy", "Baseline": b_res["totalElectricity_kwh_y"], "Option": o_res["totalElectricity_kwh_y"]},
                        {"Metric": "Water", "Baseline": b_res["waterConsumption"]["V_total_m3_y"], "Option": o_res["waterConsumption"]["V_total_m3_y"]},
                        {"Metric": "Carbon", "Baseline": b_res["carbon"]["CO2_total_kg_y"], "Option": o_res["carbon"]["CO2_total_kg_y"]},
                        {"Metric": "Opex", "Baseline": b_res["opex"]["opex_total_nzd_y"], "Option": o_res["opex"]["opex_total_nzd_y"]},
                    ])
                    st.pyplot(fig_grouped_bar_vertical(df_kpi, "KPIs: Baseline vs Option", "Value"))

                    df_energy = pd.DataFrame([
                        {"Scenario": "Baseline", "Component": "Space Heating", "Value": b_res["spaceHeating"]["Q_purchased_kwh_y"]},
                        {"Scenario": "Baseline", "Component": "Water Heating", "Value": b_res["waterHeating"]["Q_purchased_kwh_y"]},
                        {"Scenario": "Baseline", "Component": "Lighting", "Value": b_res["lighting"]["Q_total_kwh_y"]},
                        {"Scenario": "Option", "Component": "Space Heating", "Value": o_res["spaceHeating"]["Q_purchased_kwh_y"]},
                        {"Scenario": "Option", "Component": "Water Heating", "Value": o_res["waterHeating"]["Q_purchased_kwh_y"]},
                        {"Scenario": "Option", "Component": "Lighting", "Value": o_res["lighting"]["Q_total_kwh_y"]},
                    ])
                    st.pyplot(fig_stacked_bar_vertical(df_energy, "Energy breakdown (excl. plug loads)", "kWh/y"))

                    b_w = b_res["waterConsumption"]["breakdown_m3_y"]
                    o_w = o_res["waterConsumption"]["breakdown_m3_y"]
                    ...
                    df_carbon = pd.DataFrame([
                        {"Scenario": "Baseline", "Component": "Electricity", "Value": b_res["carbon"]["CO2_electricity_kg_y"]},
                        {"Scenario": "Baseline", "Component": "Water", "Value": b_res["carbon"]["CO2_water_kg_y"]},
                        {"Scenario": "Option", "Component": "Electricity", "Value": o_res["carbon"]["CO2_electricity_kg_y"]},
                        {"Scenario": "Option", "Component": "Water", "Value": o_res["carbon"]["CO2_water_kg_y"]},
                    ])
                    st.pyplot(fig_stacked_bar_vertical(df_carbon, "Operational carbon breakdown", "kgCO₂e/y"))

                    df_opex = pd.DataFrame([
                        {"Scenario": "Baseline", "Component": "Electricity", "Value": b_res["opex"]["opex_electricity_nzd_y"]},
                        {"Scenario": "Baseline", "Component": "Water", "Value": b_res["opex"]["opex_water_nzd_y"]},
                        {"Scenario": "Option", "Component": "Electricity", "Value": o_res["opex"]["opex_electricity_nzd_y"]},
                        {"Scenario": "Option", "Component": "Water", "Value": o_res["opex"]["opex_water_nzd_y"]},
                    ])
                    st.pyplot(fig_stacked_bar_vertical(df_opex, "Opex breakdown", "NZD/y"))

                    st.pyplot(fig_capex_compare(b_res["capex"]["detail_breakdown_nzd"], o_res["capex"]["detail_breakdown_nzd"]))

                    payload = {
                        "timestamp": datetime.utcnow().isoformat() + "Z",
                        "baseline": {"coefficients": get_coeffs("b"), "inputs": get_scenario("b"), "results": b_res},
                        "option": {"coefficients": get_coeffs("o"), "inputs": get_scenario("o"), "results": o_res},
                        "notes": {
                            "scope": "Early-stage decision support; not certification; not simulation.",
                            "energy_boundary": "Space heating + water heating + lighting (excludes plug loads/appliances).",
                            "water_boundary": "Indoor water includes toilets, showers, taps, plus dishwasher/washing machine water.",
