...
# =============================================================================
# MINIMAL CSS (only for KPI cards + sticky action bar)
# - Theme colors should come from .streamlit/config.toml (you already set it).
# - Keep CSS only where Streamlit theming doesn't cover (custom cards + sticky CTA).
# =============================================================================
def inject_minimal_css():
    st.markdown(
        ...
        <style>
          ...
          /* --- sticky action bar: keep the CTA visible --- */
          /* This is a best-effort sticky within the page; works well in most desktop layouts. */
          .sticky-actions{
            position: sticky;
            bottom: 0;
            z-index: 20;
...
def build_help(L):
    return {
        ...
        "wash_has": help_default_source(what="Include washing machine water use.", default=bool(L["defaults"]["washing_machine"]["hasAppliance"]), source="Model default (editable)"),
        "dish_has": help_default_source(what="Include dishwasher water use.", default=bool(L["defaults"]["dishwasher"]["hasAppliance"]), source="Model default (editable)"),
        "usage_general": help_default_source(what="Behavioural assumptions for indoor water end-uses (per person).", source="Model defaults (editable)"),
        ...
        "tariffs": help_default_source(
            ...
            default=f"Electricity {L['constants']['electricity_tariff_nzd_per_kwh_default']} NZD/kWh; Water {L['constants']['water_tariff_nzd_per_m3_default']} NZD/m³",
        ...
        "efs": help_default_source(
            ...
            default=f"Grid {L['constants']['grid_emission_factor_kgco2e_per_kwh']} kgCO₂e/kWh; Water {L['constants']['water_emission_factor_kgco2e_per_m3']} kgCO₂e/m³",
            source="MfE (2024)",
        ),
    }
...
def calculate_water_enduse(s: dict) -> dict:
    ...
    return {
        "enduse_L_y": {"Toilets": V_toilet_L_y, "Showers": V_shower_L_y, "Taps": V_tap_L_y, "Laundry": V_laundry_L_y, "Dishwasher": V_dish_L_y},
...
def init_defaults():
    ...
    for p in ["b", "o"]:
        ...
        # Defaults: No appliances (as requested)
...
def apply_code_minimum(prefix: str):
    ...
    # You already corrected this earlier: code-min should be dual flush, not single flush
...
# =============================================================================
# INPUT PANELS
# - No emoji in expander labels (your request)
# - No "Core complete / Envelope complete" text pills
# =============================================================================
...
# =============================================================================
# CHARTS (consistent colors across Baseline vs Option)
# IMPORTANT FIX:
# - Use ONE trace per category (not per scenario), and x=["Baseline","Option"]
# - This guarantees category colors stay identical between Baseline and Option
# =============================================================================
...
def plot_electricity_breakdown(b_res: dict, o_res: dict | None):
    ...
    b_vals = [b_res["spaceHeating"]["Q_purchased_kwh_y"], b_res["waterHeating"]["Q_purchased_kwh_y"], b_res["lighting"]["Q_total_kwh_y"]]
    o_vals = None
    if o_res:
        o_vals = [o_res["spaceHeating"]["Q_purchased_kwh_y"], o_res["waterHeating"]["Q_purchased_kwh_y"], o_res["lighting"]["Q_total_kwh_y"]]
    plot_breakdown_stacked("1) Electricity breakdown (Baseline vs Option)", "kWh/year", cats, b_vals, o_vals)
...
def plot_water_breakdown(b_res: dict, o_res: dict | None):
    ...
    plot_breakdown_stacked("2) Water breakdown (Baseline vs Option)", "m³/year", cats, b_vals, o_vals)
...
def plot_carbon_breakdown(b_res: dict, o_res: dict | None):
    ...
    plot_breakdown_stacked("3) Operational carbon breakdown (Baseline vs Option)", "kgCO₂e/year", cats, b_vals, o_vals)
...
def plot_opex_breakdown(b_res: dict, o_res: dict | None):
    ...
    plot_breakdown_stacked("4) Operational expenditure breakdown (Baseline vs Option)", "NZD/year", cats, b_vals, o_vals)
...
def plot_capex_breakdown(b_res: dict, o_res: dict | None):
    ...
    plot_breakdown_stacked("5) Capital expenditure breakdown (Baseline vs Option)", "NZD", cats, b_vals, o_vals)
...
with tab_calc:
    ...
    # -------------------------
    # LEFT: inputs (scroll box) + sticky action area
    # -------------------------
    with left:
        try:
        ...
        # Sticky CTA / actions (always visible)
        st.markdown('<div class="sticky-actions"><div class="inner">', unsafe_allow_html=True)
        ...
        if not st.session_state["baseline_ready"]:
            ...
            if disabled:
                st.caption("Complete required Baseline fields to enable calculation.")
            clicked = st.button("Calculate Baseline", use_container_width=True, disabled=disabled, key="btn_calc_baseline")
        ...
        if st.session_state.get("option_unlocked", False):
            ...
            if not st.session_state.get("compare_ready", False):
                ...
                clicked = st.button("Calculate & Compare", use_container_width=True, disabled=disabled, key="btn_calc_compare")
                if clicked and not disabled:
        ...
        with results_box:
            ...
            if not st.session_state.get("baseline_ready", False):
                ...
                if validate_scenario(b_now):
                    st.caption("Baseline incomplete (required fields missing).")
                else:
            ...
            if st.session_state.get("compare_ready", False):
                ...
                if validate_scenario(o_now):
                    st.caption("Option incomplete (required fields missing).")
                else:
